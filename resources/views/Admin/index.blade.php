            ...
            <table class="table rounded bg-blue text-white t-font" style="width: 60vw">
                ...
                @foreach($posts as $post)
                        <tr class="text-right">
                            ...
                            <td>
                                @if($post->is_active===0)
                                    <form action="{{url('post/approve/'.$post->id)}}" method="post">
                                    @csrf
                                         <input type="hidden" name="_method" value="PUT"/>
                                        <input type="hidden" name="status" value="0">
                                        <button class="btn text-white btn-approve btn-sm" type="submit">غیرفعال</button>
                                     </form>
                                @else
                                    <form action="{{url('post/approve/'.$post->id)}}" method="post">
                                        @csrf
                                        ...
                                        <button class="btn text-white btn-approve btn-sm" type="submit">فعال</button>
                                    </form>
                                @endif
                            </td>
